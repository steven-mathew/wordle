...
fn na(e: DateTime<Local>, a: DateTime<Local>) -> i64 {
    let s = e.date();
    let a = a.date();
...
fn da(start: DateTime<Local>, e: DateTime<Local>) -> (usize, &'static str) {
    let s = na(start, e);
    let a = (s as usize) % LA.len();

    (a, LA[a])
}

// Yes, all the variable names in here are weird. These are the names in the
// minified (or possibly, obfuscated) Wordle source code.
fn main() {
    let ha = Local.ymd(2021, 6, 19).and_hms(0, 0, 0);

    let today = chrono::offset::Local::now();
    let (_, today_word) = da(ha, today);

    println!("{} ({})", today_word, today.date());
}
